...
if(isset($_GET['prevsearch'])){
  ...
  $stmt = $dbh->prepare('select * from Book b join WrittenBy wb on b.ISBN=wb.ISBN join Author a on wb.ssn=a.ssn join Stocks s on wb.ISBN=s.ISBN where a.name=:author or b.title=:booktitle');
  $stmt->bindParam(':author', $value);
  $stmt->bindParam(':booktitle', $value);
  $stmt->execute();
  /*if($stmt->rowCount()==0){
	  echo "<span class='msg'><b>No such Author found</b></span>";
  }*/
